 ...
 const { code, bots = [], context = {} } = ev.data || {};
  try {
    ...
    const web3 = await web3Promise;
        const { rpcUrl, network, isAdvancedMode, systemState, ...restContext } = context;
    const connection = new web3.Connection(rpcUrl, 'confirmed');
    const detectedNetwork = network || (rpcUrl.includes('mainnet') ? 'mainnet-beta' : 'devnet');
    const workerContext = { ...restContext, rpcUrl, network: detectedNetwork, connection, web3, isAdvancedMode };
